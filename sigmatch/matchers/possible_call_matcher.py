...
class PossibleCallMatcher(AbstractSignatureMatcher):
    def _match(self, function: Callable[..., Any], raise_exception: bool = False) -> bool:
        ...
        3.2.1 Если в способе вызова есть именные аргументы, в сигнатуре НЕ должно быть других именных параметров, кроме перечисленных в способе вызова
        3.2.2. Если в способе вызова нет именных аргументов, сигнатура не должна содержать никаких именных аргументов
...
3.1. Сигнатура должна содержать **
3.2.1 Если в способе вызова есть именные аргументы, в сигнатуре НЕ должно быть других именных параметров, кроме перечисленных в способе вызова
3.2.2. Если в способе вызова нет именных аргументов, сигнатура не должна содержать никаких именных аргументов (обобщается с 3.2.1 как: любые обязательные именные аргументы сигнатуры должны содержаться в множестве тех, что перечислены в способе вызова)
